...
## Bokeh glow — `geometry/glow.js`

A soft additive halo over every self-lit thing in the game: a car's brake pods and blinkers, the
drive-through's lit windows and menu board, the depot's strip light, a cruiser's light bar. On by
default; `?glow=off` takes it away, and the ⚙️ panel has all four of its numbers live.

**It is not a bloom pass, and could not usefully be one.** The
[no-composer argument](#still-no-composer) has not moved — the frame's MSAA and the stencil buffer
the ghost outlines stamp into both live in the default framebuffer — but there is a second reason
here that is specific to bloom: **this game has no HDR anywhere in it.** Every emissive surface is a
`MeshBasicMaterial` or a Lambert with `emissiveIntensity` a shade over 1, so a bright-pass threshold
would find either nothing or the whole sky depending on where it was put, and the thing a player
actually wants glowing — a brake light four pixels across — is nowhere near the brightest pixel on
screen.

So the glow is **authored** rather than extracted: each lamp says how big its halo is, and the halo
is one camera-facing quad. That is not a shortcut but the answer the project had already reached
twice on its own — the helicopter's fin beacon builds a `halo` mesh around its `lamp` for precisely
this reason ([geometry/helicopter.js](#the-rooftop-helicopter--gamechopperjs-geometryhelicopterjs)),
and the off-screen police wash is the same idea drawn in CSS ([game/sirenglow.js](traffic.md)). This
generalises it and collapses every one of them into **one InstancedMesh, one material, one draw
call**.

Two properties of this camera are what make that the cheap answer rather than the lazy one:

1. **It never rotates**, so facing the camera is a constant quaternion (`BILLBOARD` in
   `game/camera.js`) rather than a per-frame lookAt. A halo costs a matrix compose.
2. **It is orthographic**, so a world-unit radius is the same number of pixels wherever the halo
   lands. "How big is this glow" is answerable once and stays answered as the player pans.

### The profile

Analytic, patched into an ordinary `MeshBasicMaterial` at `<color_fragment>` — a patch rather than a
`ShaderMaterial` because three's `USE_INSTANCING_COLOR` path is what carries a per-instance hue *and*
its brightness in one attribute. Brightness rides in the colour because `instanceColor` is RGB only,
and under an additive blend that is exact rather than a compromise: dimming an additive light **is**
multiplying its colour.

The quad's own `position.xy` is the falloff coordinate (a `MeshBasicMaterial` with no map compiles
without `USE_UV`, so there is no `vUv` to read). Two terms mixed:

| | What it is | Default |
|---|---|---|
| `falloff` | `pow(1 - r, n)` — the bloom skirt | 3.4 |
| `bokeh` | how much of a flat disc with a soft rim is mixed in | 0.10 |
| `gain` | peak alpha at the centre | 0.6 |
| `size` | a multiplier on every radius | 1.0 |

Two of those were wrong on the first pass in ways worth writing down. **`gain` at 1 is not "full
brightness", it is white**: the blend saturates long before the alpha does, so a red brake light
over grey tarmac lands at roughly (1.0, 0.89, 0.84) and the halo is a white ball wider than the car
wearing it. At 0.6 the hue survives in the middle of the halo and the white comes back on its own
where two lamps overlap — which is where a real lens actually blooms out. And **`bokeh` is very
nearly an alpha floor**, because the disc term is flat: at the 0.35 it opened on, the whole halo sat
at a third of full alpha and read as a sticker. It is worth having at 0.1 for the one thing it does,
which is put a definite edge on the halo.

### The standoff, which is a depth number and therefore invisible

`GLOW_STANDOFF` pushes each vehicle's halo **toward the camera**, and without it a halo is eaten by
its own car. The quad lies in the screen plane, so every pixel of it sits at the lamp's own
view-space depth; two points landing on the same screen pixel differ by a multiple of `VIEW_DIR`, so
a world unit of height between them is `1 / VIEW_DIR.y` = **1.83 units of depth**. A brake pod at
`LIGHT_Y` = 0.87 has its own car's roofline (2.07) standing 2.20 units in front of it, directly
up-screen of the pod on any car driving away from the camera. Flush, the top of every brake halo is
clipped off against the boot lid — and the first cut shipped at 1.6, which is exactly that.

It buys the other half of the same problem for free: the same 2.4 lets a halo hang 1.3 units *below*
the road before the tarmac in front of it cuts a chord across the disc, against the 0.92 a pod halo
actually drops.

**A vehicle's roofline is not one number**, in the same way [a road's width is not
one](city.md#divided-arterials-and-the-planted-median). A box truck's cargo box tops out at 3.50
against a car's 2.07 — 4.82 units of clearance — so on the car's standoff the box bites a flat chord
out of every truck's brake halo, and raising the shared number to cover it would push every car's
halo two and a half units forward for nothing. Two constants, each measured against its own mesh, and
`tools/probe.mjs` recomputes both from `VIEW_DIR` and the real bounding boxes rather than trusting
them.

**A light fixed to a building stands off by 0**, which is the same rule read the other way: it is
mounted on the surface that ought to occlude it. That is what keeps the depot's strip light — 1.3
units back inside its bay, 0.77 units of depth behind the shutter — dark behind a closed door, and
what makes it spill out of the opening as the door winds up without anyone animating it.

### Lanes, and why not one immediate-mode stream

Each emitter reserves a slot range once and owns it: `begin()` rewinds its own cursor, `add()` fills,
`end()` collapses whatever went dark. There is no shared cursor and therefore no frame boundary to
get wrong.

The obvious design — one stream, rewound at the top of the frame and sealed before the render — was
written first and is wrong, because **the sim does not tick once per render**. `traffic.warmup()`
runs twelve seconds of city before the first frame, [shot mode](testing.md) auto-plays whole fares
between one render and the next, and the opening vignette steps the world on its own. Every one of
those rounds emitted a full set of halos into a stream only a render would rewind, so a screenshot
came back with the buffer full and five hundred stale halos in it: lights burning in an empty street
over cars that had driven away minutes of sim time ago. Nothing logged, and the live loop — one tick,
one render — never showed it.

A lane also has to **collapse its whole range when it is allocated**, because three initialises every
instance matrix to the *identity*, not to zeros. A slot nobody has written yet is a full-size halo
standing at the world origin, and a lane sized for forty cars carries a lot of them.

### What it costs

One draw call and one material. The mesh's `count` covers every slot ever allocated — 140 of 512 at
the top of the difficulty ramp — and the unused ones are collapsed to a zero scale, which is four
degenerate vertices and not one fragment. Overdraw is the real cost and it is small: a pod halo is a
17px disc at play zoom, and a crowded frame has a couple of dozen of them.

`renderOrder` is **0, under everything**. A halo is part of the picture rather than a read-out, so
the [crayon page](#the-page--one-tile-baked-in-code) at 1 washes over it exactly as it washes over
the city, and no fare's timer ring is ever tinted by a passing indicator. Order costs nothing to give
away: additive blending is commutative and the pass writes no depth.

The mesh is `frustumCulled = false`, for [the reason the ambient
trucks](traffic.md) are — three computes an InstancedMesh's bounding sphere once, from the matrices
as they stood on the first frame it was culled, and this one moves every frame.

### What it is not doing yet

- **Street lamps and building windows do not glow**, only the drive-through's and the depot's. The
  city's windows are baked vertex colour on the merged building mesh, so there is nowhere to hang a
  per-window anchor without generating one at mesh time.
- **Nothing glows brighter for being in shadow.** The halo is the same strength on a sunlit forecourt
  as under a tower, where a real light would read far harder in the shade.
- **The day/night curve is one number for every fixed light** (`litWindows` in `main.js`), rather
  than each kind of lamp having its own switch-on hour.
