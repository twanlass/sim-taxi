...
| **sfx** | `tools/sfx.mjs` | The sound bank against a fake `AudioContext`: every voice built, stopped and disconnected, no envelope ramped exponentially to zero (a real `AudioParam` throws, and the browser swallows it), the voice cap, the repeat gap, a context that refuses to start, and the siren's loudness curve. See [audio.md](audio.md#testing) |
