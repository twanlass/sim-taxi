...
import {
  setGlow, glowField, resetGlowField, GLOW_STANDOFF, TRUCK_GLOW_STANDOFF, GLOW_CAPACITY,
  POD_GLOW_R, VEHICLE_GLOW_SLOTS,
} from '../src/geometry/glow.js';
import { LIGHT_Y } from '../src/geometry/lights.js';
...
// --- The bokeh glow ------------------------------------------------------------
//
// Every halo in the game is one additive quad in one instanced mesh (geometry/glow.js). Three
// separate things about it fail silently, and all three have already been through here once:
//
//   1. **Whether a halo stands on a lamp at all.** The first cut framed the field on the render,
//      which is wrong because the sim does not tick once per render — `warmup()` and shot mode's
//      auto-play both run dozens of rounds between two frames, and every one of them stacked a
//      fresh set of halos on top of the last until the buffer was full. On screen that is lights
//      burning in an empty street, and there is no error anywhere. Lanes fixed it; this is what
//      says so.
//   2. **The standoff.** It is a depth number, and depth is exactly what a screenshot cannot show.
//   3. **The material flags.** The additive blend, the depth write and the cache key are the whole
//      behaviour, and every one of them fails by drawing something plausible.
{
  setGlow(true);
  resetGlowField();

  const glScene = new THREE.Scene();
  const glTraffic = createTraffic(makeRng(seed + 44), glScene, 12);
  const glPolice = createPolice(makeRng(seed + 66), glScene, glTraffic.cars);
  const field = glowField();
  const mesh = field.mesh;

  check('the glow is one instanced mesh, not a pass',
    mesh instanceof THREE.InstancedMesh,
    `${mesh.count} slots, one draw call`);
  // The trap the ambient trucks paid for: three computes an InstancedMesh's bounding sphere once,
  // on the first frame it is culled, and never again. This one moves every frame.
  check('the glow mesh is never culled', mesh.frustumCulled === false);
  check('halos blend additively and write no depth',
    mesh.material.blending === THREE.AdditiveBlending && mesh.material.depthWrite === false
    && mesh.material.depthTest === true,
    'depth-tested so a building still hides one, depth-silent so two never fight');
  // The diamond's fill drew with a building's shader for want of this. See CLAUDE.md.
  check('the glow material carries a program cache key',
    typeof mesh.material.customProgramCacheKey === 'function'
    && mesh.material.customProgramCacheKey() === 'glow',
    'this project is full of unpatched MeshBasicMaterials to collide with');

  // --- The standoff, worked out rather than eyeballed.
  //
  // Two points at the same screen position under this orthographic camera differ by a multiple of
  // VIEW_DIR, so a world unit of *height* is `1 / VIEW_DIR.y` units of depth between them. A halo
  // has to step past its own car's roofline to spill over the bodywork instead of being clipped by
  // it, and past the road in front of it not to be sliced by the tarmac.
  const topOf = (name) => {
    const mesh = glScene.children.find((o) => o.name === name);
    mesh.geometry.computeBoundingBox();
    return mesh.geometry.boundingBox.max.y;
  };
  const clearance = (top) => (top - LIGHT_Y) / VIEW_DIR.y;
  const carTop = topOf('cars');
  const roofClearance = clearance(carTop);
  check('the standoff steps a halo past its own car',
    GLOW_STANDOFF > roofClearance,
    `${GLOW_STANDOFF} vs ${roofClearance.toFixed(2)} from a pod at ${LIGHT_Y.toFixed(2)} to a roof at ${carTop.toFixed(2)}`);
  // A vehicle's roofline is not one number, in the same way a road's width is not one number.
  const boxTop = topOf('truckBoxes');
  const boxClearance = clearance(boxTop);
  check("...and a truck's past its cargo box, which is a different number",
    TRUCK_GLOW_STANDOFF > boxClearance && GLOW_STANDOFF < boxClearance,
    `${TRUCK_GLOW_STANDOFF} vs ${boxClearance.toFixed(2)} to a box roof at ${boxTop.toFixed(2)}; the car's ${GLOW_STANDOFF} would not do`);
  // A halo hangs `r * |VIEW_UP.y|` below its lamp in world height — the quad lies in the screen
  // plane, so its own down direction is the camera's.
  const podDrop = (POD_GLOW_R * VIEW_UP.y - LIGHT_Y) / VIEW_DIR.y;
  check('...and past the road under it',
    GLOW_STANDOFF > podDrop,
    `${GLOW_STANDOFF} vs ${podDrop.toFixed(2)} of depth to the tarmac at the bottom of a pod halo`);

  // --- Capacity. The fleet is the ceiling, and it is a known one.
  const wanted = (difficulty.carCount(Infinity) + 1) * VEHICLE_GLOW_SLOTS + 2;
  check('the field holds the whole fleet at the top of the ramp',
    wanted <= GLOW_CAPACITY, `${wanted} of ${GLOW_CAPACITY} slots at ${difficulty.carCount(Infinity)} cars`);

  // --- Every halo stands on a lamp, however many rounds run between two frames.
  //
  // Drive it the way `warmup()` does — many updates, no render — and then ask where the halos
  // actually are. Under the bug this is the check that goes red: the buffer fills with rounds the
  // player never saw, and their halos sit over road the cars left minutes of sim time ago.
  const glPos = new THREE.Vector3();
  const strayHalo = () => {
    const arr = mesh.instanceMatrix.array;
    let worst = 0;
    let live = 0;
    for (let i = 0; i < mesh.count; i++) {
      // A retired slot is collapsed to a zero scale, not merely blacked out.
      if (Math.hypot(arr[i * 16], arr[i * 16 + 1], arr[i * 16 + 2]) < 1e-3) continue;
      live += 1;
      glPos.set(arr[i * 16 + 12], 0, arr[i * 16 + 14]).addScaledVector(VIEW_DIR, -GLOW_STANDOFF);
      let near = Infinity;
      for (const car of glTraffic.cars) {
        near = Math.min(near, Math.hypot(glPos.x - car.x, glPos.z - car.z));
      }
  ...
  for (let step = 0; step < 600; step++) glTraffic.update(1 / 60);
  const settled = strayHalo();
  // Half a car length: a pod sits on a corner, and the taxi's are TAXI_SCALE further out again.
  check('every halo stands on a vehicle after 600 rounds with no render',
    settled.worst < 2.5,
    `${settled.live} halos up, furthest ${settled.worst.toFixed(2)} from any car`);
  check('...and the field is not full',
    settled.live <= glTraffic.cars.length * VEHICLE_GLOW_SLOTS,
    `${settled.live} halos for ${glTraffic.cars.length} vehicles`);

  // The cruiser's two, framed by its own `update` rather than by `siren()` — which is not reached
  // on a tick spent in the cooldown between runs, and a lane nobody rewinds keeps what it had.
  glPolice.chase?.call?.(null);
  for (let step = 0; step < 60; step++) glPolice.update(1 / 60);
  const dark = !glPolice.state.lit;
  const after = strayHalo();
  check('a dark light bar leaves no halo behind it',
    !dark || after.live <= settled.live + 2,
    dark ? `${after.live} halos with the bar off` : 'bar is lit, nothing to assert');

  // Put the flag back: everything the probe builds after this expects the inert stub, and a field
  // left allocated would go on collecting lanes from every traffic model built below.
  setGlow(false);
  resetGlowField();
}
